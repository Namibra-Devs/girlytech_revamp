...
import { Heart, Shield, RefreshCw, Zap, Users, BookOpen, Award } from 'lucide-react'
...
const impacts = [
  { icon: BookOpen, label: 'N$200',    desc: 'Covers learning materials for one girl for a month' },
  { icon: Users,    label: 'N$500',    desc: 'Funds one girl\'s program fees for a full month' },
  { icon: Award,    label: 'N$2,000',  desc: 'Sponsors a full scholarship for one program cohort' },
  { icon: Heart,    label: 'N$10,000', desc: 'Funds an entire bootcamp for 10 girls' },
]
    ...
    <>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-wrap justify-center gap-8 text-sm text-gray-500">
          ...
          <span className="flex items-center gap-2"><Heart className="w-4 h-4 text-pink-500" />100% goes to programs</span>
        </div>
      </div>
          ...
          <div className="grid lg:grid-cols-3 gap-10 lg:gap-12">
            ...
            <div className="lg:col-span-2 bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
              ...
              <div className="relative mb-8">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-bold">N$</span>
                <input
                  type="number"
                  value={custom}
                  onChange={(e) => { setCustom(e.target.value); setSelected(0) }}
                  placeholder="Enter custom amount"
                  className="w-full pl-12 pr-4 py-3.5 border-2 border-gray-200 focus:border-pink-400 rounded-xl text-sm text-navy-950 placeholder-gray-400 focus:outline-none transition-colors"
                />
              </div>
              ...
              <button className="w-full py-4 bg-gradient-to-r from-pink-600 to-pink-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-pink-500/30 transition-all">
                <Heart className="w-5 h-5" />
                Donate N${(final || 0).toLocaleString()} {recurring ? 'Monthly' : 'Now'}
              </button>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-navy-950 rounded-3xl p-6 relative overflow-hidden">
                ...
                <div className="relative">
                  <p className="text-xs font-bold uppercase tracking-wider text-pink-400 mb-3">Total Impact</p>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {tiers.map(({ label, amount, currency, icon: Icon, color, popular, perks }) => (
              <div
                key={label}
                className={`rounded-3xl p-8 relative overflow-hidden ${popular ? 'bg-navy-950 text-white ring-2 ring-pink-500' : 'bg-gray-50'}`}
              >
                {popular && (
                  <span className="absolute top-5 right-5 px-3 py-1 bg-pink-600 text-white text-xs font-bold rounded-full">Most Popular</span>
                )}
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-5 ${popular ? 'bg-white/15' : 'bg-pink-50'}`}>
                  <Icon className={`w-6 h-6 ${popular ? 'text-pink-400' : 'text-pink-600'}`} />
                </div>
                <h3 className={`font-display font-bold text-2xl ${popular ? 'text-white' : 'text-navy-950'}`}>{label}</h3>
                <p className={`text-3xl font-black mt-1 mb-6 ${popular ? 'text-pink-400' : 'text-navy-950'}`}>
                  {currency}{amount.toLocaleString()}<span className="text-base font-normal opacity-50">/mo</span>
                </p>
                <ul className="space-y-3 mb-8">
                  {perks.map((perk) => (
                    <li key={perk} className={`flex items-start gap-2 text-sm ${popular ? 'text-white/75' : 'text-gray-600'}`}>
                      <span className="text-pink-500 mt-0.5 flex-shrink-0">✓</span> {perk}
                    </li>
                  ))}
                </ul>
